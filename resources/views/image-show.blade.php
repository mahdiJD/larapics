        ...
        <div class="row">
            ...
            <div class="col-md-3">
                ...
                <div class="d-flex justify-content-between py-3 border-top border-bottom">
                    ...
                    @auth
                    <div>
                        <x-form action="{{ route('likes.update' , $image->slug)}}"
                            style="display: inline;">
                            <button type="submit" title="Like mage"
                            class="btn @if ($image->hasBeenLiked())
                            btn-secondary
                            @else
                            btn-primary
                            @endif" >
                        ...
                        <x-form action="{{ route('favorites.update' , $image->slug)}}" style="display: inline;">
                            <button type="submit" title="Favorite mage" class="btn @if ($image->hasBeenFavorites())
                            btn-warning
                            @else
                            btn-danger
                            @endif " >
                    ...
                    <div>
                        ...
                        onclick="alert('You shold login to like this post!')">
                            <x-icon src="thumbs-up.svg" alt="" class="align-text-top" width="18" height="18" />
                            {{ $image->likes_count }}
                        </button>

                        <button type="button" title="Favorite mage" class="btn btn-danger"
                        onclick="alert('You shold login to add this post to your favorite!')">
                            <x-icon src="heart.svg" alt="" width="18" height="18" />
                        </button>
                    </div>
                    @endauth
